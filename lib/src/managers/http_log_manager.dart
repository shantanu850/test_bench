...
final class HttpLogManager {
  ...
  void onRequest(RequestBean options) {
    ...
    if (_printLogs) {
      _prettyTestBench.onRequest(options);
    }
  ...
  Future<void> onResponse(ResponseBean response) async {
    if (_printLogs) {
      await _prettyTestBench.onResponse(response);
    }
