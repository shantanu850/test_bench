...
import 'package:test_bench/src/utils/hide_values_in_map.dart';
...
final class RequestBean {
  ...
  Map<String, dynamic> toJson() {
    final changedHeaders = headers?.map((key, value) {
      return TestBenchInitializer.instance.hiddenHeaders.contains(key)
    ...
    if (params is Map) {
      changedParams = (params as Map).map(
        (key, value) => TestBenchInitializer.instance.hiddenFields.contains(key)
            ? MapEntry(key, kHidden)
            : MapEntry(key, value),
      );
    }
